...
function DashboardMockup() {
  return (
    <div className="relative w-full max-w-[560px] mx-auto select-none">
      <div className="absolute inset-0 bg-[#2E8B57]/20 blur-3xl rounded-full scale-110 pointer-events-none"/>
      <div className="relative float-anim bg-white rounded-[22px] shadow-[0_40px_100px_rgba(0,0,0,0.2)] border border-white/30 overflow-hidden">
        {/* Topbar */}
        <div className="bg-[#0B5D4B] px-5 py-3 flex items-center justify-between">
        ...
        <div className="flex h-[320px]">
          {/* Sidebar */}
          <div className="w-12 bg-[#084437] flex flex-col items-center py-4 gap-3.5">
          ...
          <div className="flex-1 bg-[#F6F8FA] p-3.5 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              {[{l:'Revenue',v:'₹84.2L',c:'#0B5D4B',d:'+12%'},{l:'Orders',v:'1,248',c:'#2E8B57',d:'+8%'},{l:'Quality',v:'98.4%',c:'#084437',d:'+3%'}].map((k,i)=>(
              ...
              <div className="flex items-end gap-1.5 h-14">
                {[40,65,50,80,60,92,74].map((h,i)=>(
                  <div key={i} className="flex-1 rounded-t-md" style={{
                    height:`${h}%`,
                    background:i===5?'#0B5D4B':`rgba(11,93,75,${0.18+i*0.07})`
                  }}/>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white rounded-xl p-2.5 shadow-sm border border-slate-100/80">
                ...
                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full w-[88%] bg-[#0B5D4B] rounded-full"/>
                </div>
                <div className="text-[10px] font-bold text-[#0B5D4B] mt-1">88% Optimal</div>
              </div>
              <div className="bg-white rounded-xl p-2.5 shadow-sm border border-slate-100/80">
                ...
                {['Prod A','Prod B','Prod C'].map((p,i)=>(
                  <div key={i} className="flex items-center gap-1 mb-1 last:mb-0">
                    <div className="w-1 h-1 rounded-full bg-[#0B5D4B] flex-shrink-0"/>
                    <span className="text-[8px] text-slate-600 flex-1">{p}</span>
                    <div className="h-1 bg-[#2E8B57] rounded-full" style={{width:`${55-i*12}px`}}/>
                  </div>
                ))}
              </div>
            </div>
      ...
      <motion.div animate={{y:[-6,6,-6]}} transition={{duration:4,repeat:Infinity}}
        ...
        <div className="w-8 h-8 bg-[#DDF5E8] rounded-xl flex items-center justify-center">
          <TrendingUp size={15} className="text-[#0B5D4B]"/>
        </div>
        <div>
      ...
      <motion.div animate={{y:[6,-6,6]}} transition={{duration:5,repeat:Infinity}}
        ...
        <div className="w-8 h-8 bg-[#DDF5E8] rounded-xl flex items-center justify-center">
          <CheckCircle2 size={15} className="text-[#0B5D4B]"/>
        </div>
        <div>
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center dark-bg pt-[72px] overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-15%] right-[-8%] w-[700px] h-[700px] rounded-full bg-[#2E8B57]/12 blur-[130px]"/>
        <div className="absolute bottom-[-10%] left-[-10%] w-[550px] h-[550px] rounded-full bg-[#0B5D4B]/20 blur-[100px]"/>
          ...
          <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-8">
            <motion.div variants={fadeUp}>
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 text-white/90 text-sm font-semibold backdrop-blur-sm">
                <Zap size={13} className="text-[#DDF5E8]"/> Next-Generation Manufacturing ERP
              </span>
            </motion.div>
            ...
            <motion.h1 variants={fadeUp}
              ...
              Manufacturing <br/>
              <span style={{background:'linear-gradient(135deg,#DDF5E8 0%,#7dd3a8 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>
                Intelligence.
              </span> <br/>Simplified.
            </motion.h1>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-4">
              <a href="#contact"
                className="inline-flex items-center gap-2 px-7 py-4 rounded-[14px] bg-white text-[#0B5D4B] font-semibold text-base hover:bg-[#DDF5E8] transition-all shadow-2xl hover:-translate-y-0.5">
